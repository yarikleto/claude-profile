...
- **Stores bind to their live paths** via `.live-paths`, written atomically under the lock on the first mutation. Compare the canonical live directory and JSON parent plus filename before writes or migration; do not bind to the target of a JSON symlink. Old unbound stores adopt the current paths on first write. `CLAUDE_CONFIG_DIR` must be absolute, and live directories cannot be `$HOME`, `/`, or an ancestor of `$HOME`.
