...
- **Stores bind to their live paths** via `.live-paths`, written atomically under the lock before storing configuration, not on invalid commands. Compare the canonical live directory and JSON parent plus filename before writes or migration; do not bind to the target of a JSON symlink. Old unbound stores with saved state refuse relocated JSON because released versions always managed `$HOME/.claude.json`; default-JSON upgrades may adopt the selected directory. `CLAUDE_CONFIG_DIR` must be absolute, and live directories cannot be `$HOME`, `/`, or an ancestor of `$HOME`.
- **A dangling managed JSON symlink is not a nonempty live state for crash recovery**. Copy-mode saves validate broken links before the emptiness check; otherwise a dangling link could cause an empty live tree to erase a recovered profile.
